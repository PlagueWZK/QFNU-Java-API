...
public record Term(int startYear, int endYear, int termIndex) {
    public Term {
        if (endYear - startYear != 1) {
            throw new IllegalArgumentException("学期id年份必须连续(如2025-2026)");
        }
        if (termIndex < 1 || termIndex > 2) {
            throw new IllegalArgumentException("学期id必须为1,2或3");
        }
    }
    ...
    public static Term parse(String termId) {
        if (termId == null || termId.isBlank()) {
            throw new QFNUAPIException("学期id不能为空");
        }
        try {
            ...
            throw new QFNUAPIException("学期id格式错误, 应该为yyyy-yyyy-n", e);
        }
    }
